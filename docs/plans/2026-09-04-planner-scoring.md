...
## refillCost surplus guard (2026-09-05)

A Go + Agile Outgoing day (2026-07-09, 32/10 contiguous, G100 5 kW, cycleLife 6000,
`laterCheaperRefill` / `refillCost`) charged 00:30-01:30, then alternated export at ~11.9p
and charge inside the cheap window, left the window at 14.7 kWh against 17 kWh of day load,
hit the floor at 20:00 and imported 2.7 kWh at 31.38p. Cause: under `refillCost` pass 1
valued the day's load at the refill cost (8.63/0.9 + 2.0 wear = 11.6p) because a refill
slot still lay ahead of it, `laterCheaperRefill` set the day-slot floor to the 36.9p day
"refill" so evening export (17-23p) was held, and the only spend left above its floor was
window export at 11.7-12.4p. Each such export also took a window slot that could no longer
charge (one-meter), and nothing checked that the remaining slots could still refill the
pack.

Fix (`js/causal.js` pass 1, mirrored in `test/causal_model.py`): under `refillCost` a spend
at value p may take only the surplus `E + R(p) − D(p)` — pack energy, plus pack-side room at
refill slots cheaper than p not already spent in, less demand worth more than p — and a
spend inside a refill slot forfeits that slot's room. `displacedPrice` is untouched (all
its fixture cases regenerated byte-identical). Unit tests: `refillCost balance` ×3 in
`test/units.mjs`; fixture `contig-go-refill` (Go prices, Agile-Outgoing-like export).

£/yr saving, 32/10 contiguous, real year, no wear, no G100 (before → after):

| holdFor / packEnergyWorth | Go+Outgoing | Agile+Outgoing | Agile no-export |
|---|---|---|---|
| anyCheaperRefill / displacedPrice (shipped) | 1132 → 1132 | 635 → 635 | 403 → 403 |
| anyCheaperRefill / refillCost | 1356 → 1356 | 657 → 657 | 474 → 480 |
| laterCheaperRefill / displacedPrice | 1291 → 1291 | | |
| laterCheaperRefill / refillCost | 1283 → **1555** | | |

The screenshot's own configuration (G100 5 kW, cycleLife 6000, `laterCheaperRefill` /
`refillCost`): £1,384.66 → £1,478.21 (wear £192 → £197); 2026-07-09 itself £2.83 → £4.82,
now filling to 32 kWh by 03:30, exporting in the evening and ending the day at 3.6 kWh with
no day-rate import. Slots exporting while export beat import: 229 → 10. On the synthetic Go
fixture year: saving £99 → £154, window-export slots 151 → 20, slots at the floor 241 → 9,
day-rate import after 19:00 63 kWh → 0. `laterCheaperRefill` / `refillCost` is now the best
Go combination; defaults unchanged pending a second household. Cost: the guard is O(T + buckets)
per pass-1 candidate, ~+1.5 s on a year's score under `refillCost`.
