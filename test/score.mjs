...
for (const ramp of list('ramp', base.rampSlots)) {
  ...
  rows.push({
    cap, inv, cycle, export: ex, alpha, lambda: lam, ramp,
  ...
  console.error(`  ${rows.length}: ${cap}kWh/${inv}kW ${cycle} export=${ex} α=${alpha} λ=${lam} ramp=${ramp} -> £${saved.toFixed(2)} (${ms.toFixed(0)} ms)`);
}
